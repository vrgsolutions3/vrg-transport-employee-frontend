            ...
            {selectedAssignments.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-1.5">
                {selectedAssignments.map((s: any, idx: number) => {
                  ...
                  return (
                    <span key={uid ?? idx} className="px-2.5 py-0.5 rounded-full bg-surface-container-high text-xs font-medium text-on-surface-variant">
                      {label}
                    </span>
                  );
                })}
            ...
            {(selected?.waitlistedCount ?? 0) > 0 && (
              <div>
                <button
                  ...
                  className="rounded-md bg-warning-container px-3 py-2 text-sm text-on-warning hover:bg-warning-container/80 transition"
                >
                  Liberar vagas
                </button>
              </div>
